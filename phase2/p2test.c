...
void print(char *msg) {
 ...
 //   klog_print("S value after P(sem_term_mut):");
  //  klog_print(s);

    // s = msg; /* in case it was changed by an interrupt handler */
    

    while (*s != EOS) {
        devregtr value = PRINTCHR | (((devregtr)*s) << 8); // = 28674
        status = SYSCALL(DOIO, (int)command, (int)value, 0);
     //   klog_print("status after DOIO:");
        klog_print_hex(status);
        if ((status & TERMSTATMASK) != RECVD) {
        //    klog_print("RISULATO CONTROLLO DEL WHILE:");
            klog_print_hex(status & TERMSTATMASK);
      //      klog_print("STO PER PANICARE\n");
            PANIC();
        }
        s++;
    }
//   klog_print("print done, Ving sem_term_mut\n");
...
void test() {
 ...
 //   klog_print("PRINTATO IL MESSAGGIO (NOT REALEMENTE) \n");
    ...
    klog_print("p4 IS STARTED WITH PID:");
    klog_print_dec(p4pid);
    klog_print("\n");
...
void p4() {
    klog_print("P4 STARTS FORREAL WITH PID:");
    klog_print_dec(p4pid);
    klog_print("\n");
    ...
    int pid = SYSCALL(GETPROCESSID, 0, 0, 0);
    // il pid ritornato è 6 quindi ok, come fa p4pid a divetare 0?
    if (pid != p4pid) {
...
void p5gen()
{
    ...
    klog_print("p5gen entered with exception code:");
    klog_print_hex(exeCode);
    klog_print("\n");
    switch (exeCode)
    {
    // store access fault
    ...
    klog_print("CAUSE DELLA ECCEZIONE IN P5GEN:");
    klog_print_hex(exeCode);
...
void p5a() {
    ...
    klog_print("p5a starts, about to cause TLB exception\n");
...
void p5b() {
    klog_print("p5b starts, about to cause address error exception\n");
    cpu_t time1, time2;
...
void hp_p1() {
    ...
    klog_print("P4 PID INSIDE HPP1 (END):");
    klog_print_dec(p4pid);
    klog_print("\n");
